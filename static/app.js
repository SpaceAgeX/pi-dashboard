...
async function api(path, options) {
  const response = await fetch(path, { ...options, headers: { Accept: "application/json", ...(options?.method && options.method !== "GET" ? {"X-PI-Dashboard":"same-origin","Content-Type":"application/json"}:{}), ...(options?.headers||{}) } });
...
function dateTime(value){return value?new Date(value*1000).toLocaleString():"--"}
async function accessAction(path,options={method:"POST"}){try{await api(path,options);await loadAccess()}catch(error){showError(error.message)}}
async function loadAccess(){const data=await api("/api/access");$("#pending-devices").innerHTML=data.pending.length?data.pending.map(item=>`<article class="device"><strong>${safe(item.approval_code)}</strong><span>${safe(item.user_agent)}</span><small>${dateTime(item.requested_at)} · ${safe(item.source_ip)}</small><div><button class="button" data-approve="${safe(item.id)}">Approve</button><button class="button danger" data-deny="${safe(item.id)}">Deny</button></div></article>`).join(""):"<p>No pending requests.</p>";$("#trusted-devices").innerHTML=data.trusted.length?data.trusted.map(item=>`<article class="device"><strong>${safe(item.name)}</strong><span>${safe(item.user_agent)}</span><small>Approved ${dateTime(item.approved_at)} · Last seen ${dateTime(item.last_seen_at)} · ${safe(item.last_ip)}</small><div><button class="button" data-rename="${safe(item.id)}">Rename</button><button class="button danger" data-revoke="${safe(item.id)}">Revoke</button></div></article>`).join(""):"<p>No trusted devices.</p>";document.querySelectorAll("[data-approve]").forEach(b=>b.onclick=()=>accessAction(`/api/access/pending/${b.dataset.approve}/approve`));document.querySelectorAll("[data-deny]").forEach(b=>b.onclick=()=>accessAction(`/api/access/pending/${b.dataset.deny}/deny`));document.querySelectorAll("[data-revoke]").forEach(b=>b.onclick=()=>confirm("Revoke this device?")&&accessAction(`/api/access/devices/${b.dataset.revoke}`,{method:"DELETE"}));document.querySelectorAll("[data-rename]").forEach(b=>b.onclick=()=>{const name=prompt("Device name");if(name)accessAction(`/api/access/devices/${b.dataset.rename}`,{method:"PUT",body:JSON.stringify({name})})})}
$("#revoke-all").onclick=()=>{if(prompt("Type REVOKE ALL to revoke every device")==="REVOKE ALL")accessAction("/api/access/devices?confirm=REVOKE%20ALL",{method:"DELETE"})};
